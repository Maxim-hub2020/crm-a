...
export const SecondaryBtn = ({ children, onClick, className = "", ...props }) => (<button onClick={onClick} className={`bg-white text-gray-700 border border-gray-200 px-5 py-2.5 rounded-full text-sm font-semibold shadow-sm hover:bg-gray-50 flex items-center justify-center gap-2 transition-colors ${className}`} {...props}>{children}</button>);
...
export const Modal = ({ title, children, onClose, width = "max-w-md" }) => (
    <div className="fixed inset-0 bg-gray-900/40 backdrop-blur-md z-[150] flex items-center justify-center p-2 sm:p-4">
        <div className={`bg-white rounded-3xl sm:rounded-[32px] shadow-2xl w-full ${width} max-h-full flex flex-col animate-in zoom-in duration-200`} onClick={e => e.stopPropagation()}>
